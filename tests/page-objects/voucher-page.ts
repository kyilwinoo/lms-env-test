...
import { voucherLocators } from '../send-voucher/voucher-locators';
    ...
    async SearchContact(customerID: string): Promise<void> {
        try {
            ...
            if (await customerItem.isVisible()) {
                ...
                const contactLink = await this.page.locator(`a[href="https://staging-carro-ticket-web.getcarsstaging.com/contacts/${customerID}"]`); //FIXME to use the url as a dynamic locator
                ...
                await this.page.getByRole('button', { name: 'Send', exact: true })
